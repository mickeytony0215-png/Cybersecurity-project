...
**冷啟動偵測**: 啟動時做兩層掃描：(1) `/proc/*/exe` 檢查直接 memfd 執行，(2) `/proc/*/cmdline` + `/proc/*/fd/*` 檢查 Python fileless loader 模式（`python3 /proc/<pid>/fd/<N>` 指向 memfd）。這樣就算 eBPF 還沒載入之前就已經植入的 malware 也能被抓到。
...
| T1070 | Indicator Removal | `/proc` 冷啟動掃描（exe + cmdline + fd）| v1/v2 | `blue_ebpf_mdr*.py` |
...
| Deploy Agent | `red_team/deploy_agent.sh` | HTTP server + 一行 curl 部署 agent (v3) | T1059.006 |
