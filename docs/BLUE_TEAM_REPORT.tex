...
fileless ICMP command-and-control, interactive command execution, and
DNS/ICMP data exfiltration -- this document explains \emph{how the Blue Team
detects and responds to each of those behaviors}, what tools implement each
control, and the scope each layer covers. We also cover
one \emph{anticipated} evasion that Project~1 did not run -- a plain TCP
reverse shell -- because it is the attacker's natural next move once the
fileless/ICMP hooks are known; it is marked as beyond-Project-1 throughout.
...
The result is a behavior-based, source-IP-agnostic kernel defense that
re-takes control at the install and C2 stages and terminates the fileless C2
in kernel space before it beacons -- substantially stronger than IP-based
blocking. A handful of stages -- covert channels built from \emph{legitimate}
syscalls, and an anticipated \co{crontab} persistence step -- sit naturally
with other sensor classes and are laid out as concrete future work. The
central lesson is that defense-in-depth is a continuous process, not a
finished product.
\end{abstract}
...
Project~1's run exercised one concrete end-to-end path,
\begin{center}
\small\ttfamily Recon $\rightarrow$ Honeypot-trigger $\rightarrow$ IP-bypass
$\rightarrow$ SSTI $\rightarrow$ Fileless ICMP C2 $\rightarrow$ Command-exec
$\rightarrow$ Exfiltration
\end{center}
so the attacker behaviors we must defend, abstracted away from the specific
exploit, are (those marked \emph{[P1]} were performed in Project~1's run;
\emph{[anticipated]} are natural follow-on moves we also handle but that
Project~1 did not run):
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Probing / target selection} against exposed services.\hfill\emph{[P1]}
  \item \textbf{Application-layer code execution} (SSTI) that turns a web
        request into arbitrary Python on the host.\hfill\emph{[P1]}
  \item \textbf{Fileless staging}: code is created as an anonymous in-memory
        file descriptor (\co{memfd\_create}) and executed via
        \co{/proc/<pid>/fd/<N>}, leaving no file on disk to scan.\hfill\emph{[P1]}
  \item \textbf{Covert command-and-control}: control traffic hidden inside
        encrypted ICMP echo payloads \emph{[P1]}; or a plain TCP reverse
        shell \emph{[anticipated]}.
  \item \textbf{Data exfiltration} over alternative protocols (Base32-over-DNS,
        hex-over-ICMP).\hfill\emph{[P1]}
  \item \textbf{Persistence} via \co{crontab} that re-downloads the agent.\hfill\emph{[anticipated]}
\end{itemize}
Project~1's documented run had \emph{only the network layer live} (its report
states eBPF validation was out of scope), so once the IP block was bypassed
the chain from SSTI onward completed unopposed. The defensive contribution of
this report is the kernel (eBPF) layer that re-takes control at the
fileless-staging and C2 steps.
...
T1059.006 & Python loader exec from \co{/proc/fd} & \co{sys\_enter\_execve} (argv) & Kernel & Enforce (kill)\\
...
\midrule
\multicolumn{5}{@{}l}{\emph{Anticipated extensions (not in Project~1's run; validated in our own round R5)}}\\
T1571$^\dagger$ & Non-Standard Port & \co{sys\_enter\_connect} & Kernel & Alert\\
T1059.006$^\dagger$ & Reverse-shell fd hijack & \co{sys\_enter\_dup2/dup3} & Kernel & Enforce (kill)\\
T1053.003$^\dagger$ & Persistence: Cron & (not detected) & -- & Gap\\
\bottomrule
\end{tabularx}

\smallskip
\footnotesize $^\dagger$Anticipated attacker evasions the stack also covers;
\emph{not} performed in Project~1's documented run. The reverse-shell hooks
were exercised in our own validation round R5.
\end{table}

The Project~1 rows above the divider are the techniques actually performed;
the two ``Gap'' rows among them (T1190, T1048.003) are not oversights but the
scope boundary of this defense -- the stages other layers own, detailed in \S\ref{sec:limits}.
...
Attacker & Red Team & recon, C2, exfil (out of scope here)\\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{tikzpicture}[font=\footnotesize, >={Latex[length=2mm]},
  ...
  \draw[->] (s0) -- node[above,font=\scriptsize]{dup2(sock,0)} (s1);
  \draw[->] (s1) -- node[above,font=\scriptsize]{dup2(sock,1)} (s2);
  \draw[->] (s2) -- node[above,font=\scriptsize]{dup2(sock,2)} (s3);
...
\begin{tikzpicture}[font=\footnotesize,
  ...
  \node[gap,  right=4mm of o5] (o6) {full-stack gap\\ (DNS exfil)};
  \foreach \n/\r/\a in {o1/R1/honeypot probe, o2/R1c/IP alias,
                         o3/R3/cold start, o4/R4/SSTI,
                         o5/{R5$^\dagger$}/reverse shell, o6/R6/DNS exfil}
...
$^\dagger$R5 (reverse shell) is an anticipated evasion we validated ourselves;
it was \emph{not} part of Project~1's run.}
\label{fig:rounds}
\end{figure}
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item \textbf{Real-time interception (R4).} Attacker re-runs the SSTI. The
        ...
        path works.}
  \item \textbf{Reverse shell (R5 -- anticipated, not in Project~1).}
        Project~1's C2 was ICMP; the natural next evasion, once the
        fileless/ICMP hooks are known, is a TCP reverse shell (no memfd, no
        raw ICMP), which leaves the four staging hooks silent by design. We
        validated this case ourselves: the same tool still catches it ---
        \co{connect} raises an alert, and the process is killed the moment
        \co{dup2/dup3} redirects all three standard fds to the socket
        (\co{0x07}). \emph{Caught on the highest-confidence signal, no version
        change required.}
  \item \textbf{DNS exfiltration (R6).} In Project~1 the attacker pulls an
        exfil agent over HTTP, runs it in the background (\co{nohup}), and
        ships data over DNS. It uses only \co{socket(SOCK\_DGRAM)} +
        \co{sendto} -- all legitimate syscalls -- so the eBPF MDR raises
        \emph{no alert}, and the exfiltration completes. \emph{Full-stack gap
        (\S\ref{sec:limits}).} (A \co{crontab}-based persistence that
        re-downloads the agent is an anticipated escalation no control here
        watches; Project~1 did not plant one.)
\end{enumerate}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Fileless ICMP C2} -- stopped at Layer~2 both \emph{cold}
        ...
        established.
  \item \textbf{TCP reverse shell} (anticipated, not in Project~1) --
        stopped at the \co{dup2/dup3} fd-hijack, independent of the C2 port.
\end{itemize}
...
\subsection{Boundaries handed to other layers}
Two stages of Project~1's chain sit outside the kernel layer's scope by
design: the SSTI entry itself (an application/WAF concern) and DNS/ICMP
exfiltration over legitimate syscalls (an egress-analytics concern). A
potential \co{crontab} persistence step -- which Project~1 did not plant --
belongs to a host-integrity sensor. Each is mapped to concrete future work
in the next section.
...
\label{sec:limits}
% =====================================================================
Every control has a defined scope. The items below mark where each layer's
responsibility ends and another sensor's begins, and feed directly into the
future-work plan of the next section.

\subsection{Network layer: IP blocking is reactive and bypassable}
\label{sec:l1-limits-detail}
By design the network block is keyed on the source IP, which the attacker
controls (\S\ref{sec:net-limits}); IP aliasing, NAT, and proxy chains
generalize the bypass. Its role is early signal and friction, with
enforcement delegated to Layer~2.

\subsection{Kernel layer: legitimate-syscall covert channels are invisible}
\label{sec:exfil-limits}
The kernel hooks match \emph{specific} dangerous syscalls and arguments. DNS
exfiltration uses ordinary \co{socket(AF\_INET, SOCK\_DGRAM)}/\co{sendto} to
port~53, with the payload Base32/Base64-encoded inside subdomains -- traffic
no syscall-level hook is meant to flag. This is the canonical case for a
different sensor class: egress/DNS analytics rather than syscall hooks, added
as the highest-value item in future work.

\subsection{Temporal gap: hooks only see new syscalls}
\label{sec:gap-limits}
Tracepoints fire forward in time, so activity predating MDR start is outside
the hot path. The \co{/proc} cold-start scan (\S\ref{sec:coldstart}) covers
the common case; continuous re-scanning and earlier deployment extend it
further.

\subsection{False positives and whitelist tuning}
\label{sec:fp-limits}
Several enforcing hooks have benign analogues (\co{memfd\_create},
\co{execve} from \co{/proc/fd}, all-three-fd \co{dup2}), which is why
enforcement is restricted to the highest-confidence patterns and the rest are
alert-only. Moving from single-signal kills to a correlation score, and a
dynamic suspect-port list (future work), tightens this further.

\subsection{Active-response risk}
\label{sec:ar-limits}
Because an in-kernel \co{SIGKILL} is irreversible, monitor-only is the
default and the enforcing hooks fire only on the highest-confidence signals.
Adding a confirm/quarantine step and confidence scoring (future work) reduces
the residual risk further.

\subsection{Brittleness of argument matching}
\label{sec:brittle-limits}
The matchers are deliberately precise (literal \co{/proc/} prefix, fixed
\co{argv} offset, exact fd~0/1/2 for the reverse shell), shaped by the
verifier's bounded-loop and fixed-offset constraints. Path normalization and
a wider argv scan (future work) broaden coverage against deliberate evasion.

\subsection{Persistence (anticipated, out of scope)}
\label{sec:persist-limits}
Project~1 ran its exfil agent as a \co{nohup} background process, not from
\co{cron}, so no persistence was planted. A \co{crontab}-style relaunch
belongs to a host-integrity sensor that watches autostart surfaces
(\co{cron}, \co{systemd} units, rc files); eradication of the launcher is
listed as future work.

\subsection{Monitoring-plane and operational limits}
\label{sec:misc-limits}
The enforcing kill is in-kernel and unaffected, but the userspace alert path
(256\,KB perf buffer, flat-file \co{soc\_events.jsonl}, in-memory timeline)
can drop events under burst and has no durable retention or cross-host
correlation. eBPF also needs root and a BCC runtime compile, and C2 content
is AES-encrypted so the defense reasons about \emph{behavior} rather than
content. A CO-RE/libbpf rebuild, a durable SIEM, and alert scoring are the
operability items in future work.

\subsection{Root cause sits at the application layer}
\label{sec:rootcause}
These controls detect and limit the \emph{consequences} of the intrusion;
the durable fix for the SSTI entry (T1190) and root execution lives at the
application layer -- patching the injectable endpoint, dropping privileges,
and fronting it with a WAF, so Layer~2 acts as a backstop rather than the
only barrier (future work).

% =====================================================================
\section{Hardening Recommendations and Future Work}
% =====================================================================
Mapped to the scope items above:
\begin{enumerate}[leftmargin=1.4em]
...
\section{Conclusion}
% =====================================================================
The Blue Team stack shows what layered, behavior-based defense buys you.
Layer~1 (honeypot + \co{iptables}) produces clean, early signal and
sub-second blocking. Layer~2 (eBPF) is the real enforcement:
source-IP-agnostic, fileless-visible, and able to kill a process in kernel
space before a dangerous syscall completes -- it stopped Project~1's fileless
ICMP C2 (cold and hot), and the same hooks catch a TCP reverse shell at the
\co{dup2/dup3} fd-hijack (the attacker's anticipated next move, which
Project~1 did not run). A few stages -- covert channels built from legitimate
syscalls, and persistence -- sit with other sensor classes and are mapped to
concrete future work. Stated from the defender's side, the project's thesis
holds: defense-in-depth is a continuous process -- each layer is expected to
fail against some class of attack, and security comes from making sure the
next layer, and the next sensor, is already watching the next stage.
