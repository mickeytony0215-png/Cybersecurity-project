...
We describe the exact detection logic of each eBPF hook (including the v2
reverse-shell upgrade in \co{blue\_ebpf\_mdr\_v2.py}), the cold-start
...
T1620 & Reflective Code Loading & \co{sys\_enter\_memfd\_create} & Kernel v1 & Alert + taint\\
T1059 & Exec from \co{/proc/fd} & \co{sys\_enter\_execve} (argv) & Kernel v1 & Enforce (kill)\\
T1095 & Non-App-Layer (ICMP) & \co{sys\_enter\_socket} (raw ICMP) & Kernel v1 & Enforce if correlated\\
T1571 & Non-Standard Port & \co{sys\_enter\_connect} & Kernel v2 & Alert\\
T1059.006 & Reverse-shell fd hijack & \co{sys\_enter\_dup2/dup3} & Kernel v2 & Enforce (kill)\\
...
\co{blue\_team/blue\_ebpf\_mdr.py} & Kernel & eBPF v1: memfd, execve, raw-ICMP detection + correlation\\
\co{blue\_team/blue\_ebpf\_mdr\_v2.py} & Kernel & eBPF v2: adds connect (suspect ports) + dup2/dup3 reverse-shell\\
...
\subsection{v1 detections}
\co{blue\_ebpf\_mdr.py} installs four probes.
...
multi-indicator correlation is the v1 design's strongest idea: it raises
...
v1 tool compensates with a one-shot \co{/proc} walk at startup that finds
...
\subsection{v2 detections (the reverse-shell upgrade)}
The reverse shell in the exercise uses only ordinary syscalls
(\co{socket(SOCK\_STREAM)} $\to$ \co{connect} $\to$ \co{dup2} $\to$
\co{pty.spawn}) and therefore \emph{completely bypasses v1}.
\co{blue\_ebpf\_mdr\_v2.py} retains all v1 hooks and adds three.
...
# Lab T2 -- Layer 2 kernel MDR (start v1, upgrade to v2 as needed)
...
\begin{tikzpicture}[font=\footnotesize,
  ...
  \node[gap,  right=4mm of o4] (o5) {v1 blind\\ (no hook)};
  \node[good, right=4mm of o5] (o6) {L2 v2 kill\\ (dup2)};
  \node[gap,  right=4mm of o6] (o7) {full-stack gap\\ (DNS+cron)};
  \foreach \n/\r/\a in {o1/R1/honeypot probe, o2/R1c/IP alias,
                         o3/R3/cold start, o4/R4/SSTI replay,
                         o5/R5/reverse shell, o6/R6/v2 upgrade,
                         o7/R7/persistent exfil}
     \node[above=2mm of \n, align=center, font=\scriptsize] {\textbf{\r}\\ \a};
  \draw[->,thick] ([yshift=-3mm]o1.south west) -- ([yshift=-3mm]o7.south east);
\end{tikzpicture}}
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item \textbf{IP-alias bypass (R1c).} Attacker adds an alias IP and reaches
        ...
        off to Layer 2.}
  \item \textbf{Cold-start cleanup (R3).} Blue Team starts eBPF v1
  ...
  \item \textbf{Real-time interception (R4).} Attacker re-runs the SSTI. The
        ...
        path works.}
  \item \textbf{Reverse-shell bypass (R5).} Attacker switches to a TCP
        reverse shell (no memfd, no raw ICMP). v1 has no matching hook and
        \emph{stays silent}. \emph{Known v1 blind spot.}
  \item \textbf{v2 upgrade (R6).} Blue Team swaps in v2. The shell now trips
        \co{connect} (alert) and is killed when \co{dup2/dup3} hijacks the
        three fds. \emph{Layer 2 upgraded; bypass closed.}
  \item \textbf{Persistent DNS exfiltration (R7).} A \co{crontab} planted
        earlier re-downloads an exfil agent that ships data over DNS. It uses
        only \co{socket(SOCK\_DGRAM)} + \co{sendto} -- all legitimate
        syscalls -- so v2 raises \emph{no alert}, and the persistence
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Fileless ICMP C2} -- stopped at Layer~2 both \emph{cold}
        ...
        established.
  \item \textbf{TCP reverse shell} -- stopped by v2 at the \co{dup2/dup3}
...
hot) and, after the v2 upgrade, the TCP reverse shell. Yet the same design
...
\begin{tabularx}{\linewidth}{@{}l l Y@{}}
\toprule
\textbf{Tracepoint} & \textbf{Version} & \textbf{Purpose}\\
\midrule
\co{sched/sched\_process\_fork} & v1/v2 & propagate memfd taint to children\\
\co{syscalls/sys\_enter\_memfd\_create} & v1/v2 & fileless staging (alert + taint)\\
\co{syscalls/sys\_enter\_execve} & v1/v2 & exec from \co{/proc/fd} (kill on argv)\\
\co{syscalls/sys\_enter\_socket} & v1/v2 & raw ICMP socket (kill if correlated)\\
\co{syscalls/sys\_enter\_connect} & v2 & connect to suspect port (alert)\\
\co{syscalls/sys\_enter\_dup2} & v2 & reverse-shell fd hijack (kill at 0x07)\\
\co{syscalls/sys\_enter\_dup3} & v2 & dup3 variant of the above (kill at 0x07)\\
\bottomrule
\end{tabularx}
\end{center}
