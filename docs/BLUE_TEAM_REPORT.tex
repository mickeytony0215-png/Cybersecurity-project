...
control, and, in detail, \emph{where the defense still fails}. We also cover
...
\emph{legitimate} syscalls (the DNS/ICMP exfiltration Project~1 actually ran)
survive the full defense stack, and persistence (e.g.\ \co{crontab}) is an
anticipated next step no control here watches. The central lesson is that
...
honest boundary of this defense, analyzed in \S\ref{sec:limits}.
...
In Project~1's run, two techniques traverse the entire stack untouched: SSTI
itself (the root cause) and DNS/ICMP exfiltration (legitimate syscalls). A
third -- \co{crontab} persistence -- is an anticipated escalation no control
here watches, although Project~1 did not plant one. These define the next
section.
...
Project~1 did not plant persistence (its exfil agent ran as a \co{nohup}
background process, not from \co{cron}), but it is the obvious next
escalation, and the stack would not stop it: a \co{crontab} that re-downloads
and re-runs the agent on a timer would survive every kill, because killing a
process does not remove the mechanism that launches it. No control here
watches \co{cron}, \co{systemd} units, \co{.bashrc}, or other autostart
surfaces. Eradication, not just termination, is missing.
...
dangerous syscall completes -- it stopped Project~1's fileless ICMP C2 (cold
and hot), and the same hooks catch a TCP reverse shell at the \co{dup2/dup3}
fd-hijack (the attacker's anticipated next move, which Project~1 did not run).
Yet the same design that makes it strong -- enumerating specific dangerous
syscalls -- makes it blind to malice built from legitimate syscalls: Project~1's
DNS/ICMP exfiltration survived the entire stack, and a \co{crontab} persistence
mechanism would too. The honest takeaway is the project's thesis stated from
the defender's side:
