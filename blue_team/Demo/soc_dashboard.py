#!/usr/bin/env python3
"""
soc_dashboard.py - Real-time SOC Monitoring Dashboard (Fixed Path Version)
"""
import os
import sys
import json
...
body { background: #0a0e17; color: #c8d6e5; font-family: 'Consolas', 'Monaco', monospace; font-size: 14px; overflow: hidden; height: 100vh; }
.header { background: linear-gradient(135deg, #0f1923 0%, #1a2332 100%); border-bottom: 2px solid #1e90ff; padding: 16px 24px; display: flex; align-items: center; gap: 16px; }
...
.header .dot { width: 10px; height: 10px; border-radius: 50%; background: #00ff88; animation: pulse 1.5s infinite; }
...
.timeline-header { padding: 8px 24px; color: #6b7280; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; border-bottom: 1px solid #1f2937; display: grid; grid-template-columns: 90px 100px 90px 140px 120px 80px 1fr; gap: 8px; background: #0f1923; }
.timeline { flex: 1; overflow-y: auto; padding: 0 24px; height: calc(100vh - 240px); }
.event-row { display: grid; grid-template-columns: 90px 100px 90px 140px 120px 80px 1fr; gap: 8px; padding: 8px 0; border-bottom: 1px solid #111827; animation: fadeIn 0.3s ease; }
...
.empty-state { text-align: center; padding: 60px; color: #374151; font-size: 16px; }
...
<div class="stats">
    <div class="stat-card events"><div class="label">Total Events</div><div class="value" id="stat-total">0</div></div>
    <div class="stat-card blocked"><div class="label">Blocked IPs</div><div class="value" id="stat-blocked">0</div></div>
    <div class="stat-card kills"><div class="label">Process Kills</div><div class="value" id="stat-kills">0</div></div>
    <div class="stat-card critical"><div class="label">Critical Alerts</div><div class="value" id="stat-criticals">0</div></div>
</div>
<div class="timeline-header">
    <span>TIME</span><span>SOURCE</span><span>SEVERITY</span><span>EVENT</span><span>TARGET</span><span>ACTION</span><span>DETAIL</span>
</div>
...
def main():
    ...
    ap.add_argument('--port', type=int, default=8080, help='Dashboard web port (default 8080)')
    ap.add_argument('--host', default='0.0.0.0', help='Bind address (default 0.0.0.0)')
    ap.add_argument('--trap-log', default=_default_trap_log, help='Honeypot trap.log path')
    ap.add_argument('--soc-log', default=_default_soc_log, help='SOC events JSONL path')
    ap.add_argument('--poll', type=float, default=0.5, help='Log poll interval')
    args = ap.parse_args()
    ...
    t = threading.Thread(target=watcher_loop, args=(watchers, args.poll), daemon=True)
    t.start()
